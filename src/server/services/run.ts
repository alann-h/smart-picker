...
import type { Run, RunStatus, RunWithDetails, RunItemFromDB, QuoteInRun } from '~/types/run';
import type { OrderStatus } from '~/types/quote';

import { tokenService } from './token';

// Mock quote service - you'll need to implement this based on your existing services
const ensureQuotesExistInDB = async (orderedQuoteIds: string[], companyId: string, connectionType: ConnectionType): Promise<void> => {
  // TODO: Implement based on your existing quote service
  // This should ensure all quotes exist in the database before creating a run
  console.log('Ensuring quotes exist in DB:', { orderedQuoteIds, companyId, connectionType });
};

async function getNextRunNumber(companyId: string): Promise<number> {
  const result = await db.run.aggregate({
    where: { companyId },
    _max: { runNumber: true },
  });
...
export async function getRunsByCompanyId(companyId: string): Promise<RunWithDetails[]> {
  try {
    ...
    const items = await db.runItem.findMany({
    ...
    for (const item of items) {
      ...
      itemsByRunId.get(item.runId)!.push({
        ...
        orderStatus: item.quote.status as OrderStatus,
      });
    }
